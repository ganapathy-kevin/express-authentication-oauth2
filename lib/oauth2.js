		...
		if (!req.query.state) {
			...
			options.agent
				...
				.end(function responded(err, upstream) {
					...
					// KG modified 2017-04-02 to be compatible with Facebook v2.3, change to JSON response from encoded URL
					for (var body in upstream.body) break;
					var upstream_json = JSON.parse(body);
					...
					} else if (_.has(upstream_json, 'error')) {
						req.authenticated = false;
						req.authentication = {
							error: upstream_json.error,
							description: upstream_json.error_description
						};
						next();
					} else if (_.has(upstream_json, 'access_token')) {
						req.authenticated = true;
						req.authentication = {
							token: upstream_json.access_token,
							expires: expires(upstream_json)
						};
						next();
					} else {
						next({ status: 502, error: 'INVALID_UPSTREAM' });
